...
class RetrievalPlan(BaseModel):
    ...
    # Graph anchors — kept separate because the underlying tools anchor
    # differently: ticket-scoped tools (get_ticket_relations_tool,
    # traverse_ticket_network_tool, get_all_connected_nodes_content_tool)
    # take a Jira issue key, while count_tickets_by_metadata_tool takes a
    # metadata value (person/track/label/system name) + relation type.
    graph_anchors: list[ResolvedEntity] = Field(default_factory=list)
    ticket_anchor: Optional[str] = None
    metadata_anchor_value: Optional[str] = None
    metadata_relation_type: Optional[str] = None

    graph_operations: list[str] = Field(default_factory=list)
    content_relations: list[str] = Field(default_factory=list)  # relation types for get_all_connected_nodes_content_tool
    expand_node_details: bool = False  # whether to chase relation targets into get_node_details_tool
